...
用户可在“存档管理”中登录 Google Drive。应用只申请 `drive.appdata` 权限，存档位于该应用专属的隐藏目录；登录期间，每次 AI 回复完整接收，以及删除聊天、修改标签等存档变更完成后，都会安全触发增量同步。访问令牌仅保存在页面内存中；页面刷新后会尝试自动恢复连接，Google 会话失效、授权窗口被拦截或令牌过期时需要重新登录。

同步采用增量存档格式：每个对话保存为独立文件，删除标记单独保存。每次同步均按“读取 Drive Changes → 下载并合并远端变化 → 仅上传哈希发生变化的对话”执行。首次使用会自动读取旧版全量存档并迁移。
