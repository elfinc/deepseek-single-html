...
用户可在“存档管理”中登录 Google Drive。应用只申请 `drive.appdata` 权限，存档位于该应用专属的隐藏目录；登录期间，每次 AI 回复完整接收后会触发一次增量同步。访问令牌仅保存在页面内存中；页面刷新后会尝试自动恢复连接，Google 会话失效、授权窗口被拦截或令牌过期时需要重新登录。
